...
document.addEventListener('keypress', function( event ) {
    ...
    if( event.key == 'w') {
        if ( activeIndex <= WIDTH) {
            activeIndex = SQUARES - (WIDTH - activeIndex);    
        }
        else {
    ...
    if( event.key == 's') {
        if ( activeIndex >= (SQUARES - WIDTH)) {
            activeIndex = WIDTH - (SQUARES - activeIndex) ;
        }
...
document.addEventListener('click', function( event ) {
    ...
    y = Math.trunc((event.pageY - rect.y)/20);
    x = Math.trunc((event.pageX - rect.x)/20)
    activeIndex = (y * WIDTH) + x;
